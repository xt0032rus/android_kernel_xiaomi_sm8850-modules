...
#define P2PHC_MAX_PACKET_SIZE (1200)
	...
	struct net_device *dev)
{
	...
	if (skb->len > P2PHC_MAX_PACKET_SIZE)
		return NF_ACCEPT;
	...
	const struct nf_hook_state *state)
{
	...
	if (dlc == NULL) {
		p2phc_err("%s hc rx err, type:%02x", __func__, dl_type);
		return NF_DROP;
	}
	...
	if (dl_type == DL_TYPE_COMPRESSED_TCP || dl_type == DL_TYPE_COMPRESSED_UDP) {
		...
		if (skb_tailroom(skb) < HDR_LEN_MAX) {
			p2phc_err("%s hc uncompress err tailroom", __func__);
			verdict = NF_DROP;
			goto err;
		}
