	...
	struct sde_hw_ctl *ctl, struct sde_encoder_wait_info *wait_info, int wait_type)
{
	...
	while (ret == -ETIMEDOUT && timeout_iters--) {
		if (!phys_enc || !phys_enc->parent->crtc) {
			SDE_ERROR("invalid enc or crtc(%p,%p)\n", phys_enc, phys_enc->parent->crtc);
			break;
		}
...
{
	unsigned int dpu_min_ms, nominal_vsync_ms, vrr_min_idle_time_ms, frame_interval_ms;
	struct sde_connector *sde_conn;
	...
	if (sde_conn->frame_interval <
		sde_conn->freq_pattern->freq_stepping_seq[0]) {
		frame_interval_ms = DIV_ROUND_UP(1000000, sde_conn->frame_interval);
		vrr_min_idle_time_ms = frame_interval_ms + nominal_vsync_ms;
		SDE_EVT32(frame_interval_ms, vrr_min_idle_time_ms);
	}
	...
	enum sde_enc_rc_events sw_event)
{
...
#ifdef MI_DISPLAY_MODIFY
	unsigned int frame_time_ms, fps, idle_pc_frame_num;
	...
	if (lp == SDE_MODE_DPMS_LP2)
		...
		idle_pc_duration = max(idle_pc_frame_num * frame_time_ms, min_duration);
...
static void _sde_encoder_rc_cancel_delayed(struct sde_encoder_virt *sde_enc,
	u32 sw_event)
{
	SDE_EVT32(SDE_EVTLOG_FUNC_ENTRY, sw_event);
		...
		u32 sw_event)
{
	...
	SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->idle_pc_enabled,
...
#ifdef MI_DISPLAY_MODIFY
	if (!sde_enc->cur_master || !sde_enc->cur_master->connector) {
		SDE_ERROR("invalid cur_master or connector\n");
		return;
	}
#endif
...
void sde_encoder_virt_reset(struct drm_encoder *drm_enc)
{
	...
	if (!sde_enc || !sde_enc->crtc) {
		SDE_ERROR("invalid encoder or crtc\n");
		return;
	}
	sde_crtc_state = to_sde_crtc_state(sde_enc->crtc->state);
#else
	...
	SDE_EVT32(DRMID(drm_enc), sde_encoder_in_clone_mode(drm_enc));
	mutex_lock(&sde_enc->enc_lock);
	sde_enc->cur_master = NULL;
	/*
...
static void sde_encoder_virt_disable(struct drm_encoder *drm_enc)
{
	...
	SDE_EVT32(DRMID(drm_enc), sde_encoder_in_clone_mode(drm_enc), 0xbbb);
	if (!sde_encoder_in_clone_mode(drm_enc)) {
		drm_for_each_encoder_mask(encoder, drm_enc->dev,
...
static void sde_encoder_wait_for_esync_vsync(struct sde_encoder_phys *phys)
{
	...
	bool video_mrr_psr_support = false;
	...
	video_mrr_psr_support = sde_enc->disp_info.vrr_caps.video_mrr_support |
		sde_enc->disp_info.vrr_caps.video_psr_support;
	...
	SDE_EVT32(video_mrr_psr_support, empulse_notification_sim, empulse_irq_refcount);

	if (video_mrr_psr_support && empulse_notification_sim &&
	...
	} else if (video_mrr_psr_support && empulse_irq_refcount > 1)
		...
		bool config_changed)
{
	...
	#ifdef MI_DISPLAY_MODIFY
	{
		if (sde_enc->cur_master && sde_enc->cur_master->connector) {
			...
			if (sde_encoder_is_dsi_display(&sde_enc->base)) {
				display = (struct dsi_display *)c_conn->display;
				if (display && display->panel) {
					if (display->panel->mi_cfg.delay_setbacklight_flag ==true ){
						mutex_lock(&display->panel->panel_lock);
						dsi_panel_update_backlight (display->panel,display->panel->mi_cfg.delay_bl_lvl);
						mutex_unlock(&display->panel->panel_lock);
						display->panel->mi_cfg.delay_setbacklight_flag = false;
					}
				}
			}
		}
		...
		bool send_still_cmd)
{
	...
	if (!sde_enc || !sde_enc->cur_master || sde_enc->rc_state == SDE_ENC_RC_STATE_IDLE)
		return;
...
void _sde_encoder_delay_kickoff_processing(struct sde_encoder_virt *sde_enc)
{
	...
	current_ts = ktime_get_ns();
#ifdef MI_DISPLAY_MODIFY
	u64 avr_step = DIV_ROUND_UP(NSEC_PER_SEC, avr_step_fps);
	/* ept is in ns and avr_step is mulitple of refresh rate */
	ept_ts = avr_step_fps ? ept - avr_step + NSEC_PER_MSEC
...
#ifdef MI_DISPLAY_MODIFY
		if (c_conn->should_drm_set_backlight) {
			c_conn->should_drm_set_backlight = false;
			if (current_ts < ept-NSEC_PER_MSEC) {
				/*1 ms is enough*/
				return;
			} else {
				for (int i = 1; i < 5 ; i++) {
					if (current_ts < ept+i*avr_step-NSEC_PER_MSEC) {
						timeout_us = DIV_ROUND_UP((ept+i*avr_step-3*NSEC_PER_MSEC/2 - current_ts), 1000);
						if (timeout_us > 800 && timeout_us < 3*USEC_PER_MSEC) {
							snprintf(atrace_buf, sizeof(atrace_buf), "schedule_timeout_delay_%llu", timeout_us);
							SDE_ATRACE_BEGIN(atrace_buf);
							usleep_range((timeout_us - 500), timeout_us);
							SDE_ATRACE_END(atrace_buf);
						}
						return;
					}
...
#ifdef MI_DISPLAY_MODIFY
	next_vsync_time_ns = avr_step + phy_enc->last_vsync_timestamp;
#else
