		...
		struct sde_rect *rc_roi)
{
	...
	if (r1_enable &&
		...
		SDE_ERROR("idx:%d r1:%d y:%u h:%u, cfg_param_01:%d\n",
				RC_IDX(hw_dspp), r1_enable, rc_roi->y, rc_roi->h, rc_mask_cfg->cfg_param_01);
		return -EINVAL;
	}
	...
	if (r2_enable &&
		...
		SDE_ERROR("idx:%d r1:%d y:%u h:%u, mask_h:%lld, cfg_param_02:%d\n",
					RC_IDX(hw_dspp), r2_enable, rc_roi->y, rc_roi->h, mask_h, rc_mask_cfg->cfg_param_02);
		return -EINVAL;
	}
