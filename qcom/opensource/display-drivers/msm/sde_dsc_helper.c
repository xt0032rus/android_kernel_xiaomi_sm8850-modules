...
int sde_dsc_populate_dsc_config(struct drm_dsc_config *dsc, int scr_ver) {
	...
	if ((dsc->dsc_version_major == 0x1) &&
		...
		if(is_use_nt37707a_dsc_config(mi_panel_id))
			dsc->first_line_bpg_offset = 12;
	}
...
#ifdef MI_DISPLAY_MODIFY
		if (is_use_nt37707a_dsc_config_apspr(mi_panel_id) || is_use_nt38773_dsc_config_apspr(mi_panel_id)) {
