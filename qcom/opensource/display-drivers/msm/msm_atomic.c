...
static bool
_msm_seamless_for_emsync_fps_switch(struct drm_device *dev, struct drm_connector_state *conn_state)
{
	struct msm_display_mode *msm_mode;
	struct msm_drm_private *priv = dev->dev_private;
	struct msm_kms *kms = priv->kms;

	if (!dev || !conn_state || !conn_state->crtc)
		return false;
	...
	msm_mode = kms->funcs->get_msm_mode(
		_msm_get_conn_state(conn_state->crtc->state));
...
msm_disable_outputs(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	for_each_old_connector_in_state(old_state, connector,
		...
		if (_msm_seamless_for_emsync_fps_switch(dev, old_conn_state))
			continue;
