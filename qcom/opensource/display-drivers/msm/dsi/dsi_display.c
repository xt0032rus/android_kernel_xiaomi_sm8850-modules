...
#ifdef MI_DISPLAY_MODIFY
const char *mi_dsi_display_get_cmdline_panel_info(struct dsi_display *display)
{
		...
		int power_mode, void *disp)
{
	...
	DISP_TIME_INFO("Display (%s), Power mode (%s), PSR Support:%d\n", display->display_type,
			get_display_power_mode_name(power_mode),display->panel->vrr_caps.video_psr_support);
#endif
	switch (power_mode) {
	...
	case SDE_MODE_DPMS_ON:
		...
		if(!rc){
				...
				if (is_mi_dev_support_nova()) {
					DSI_INFO("[ESD] call vis_mi_esd_recover_usecase\n");
				   ...
				   struct dsi_display_mode *dsi_mode)
{
	...
	if (!dfps_caps.dfps_support &&
	    !dyn_clk_caps->maintain_const_fps &&
	    !emsync_fps_update) {
		DSI_ERR("dfps or constant fps or emsync fps switch not supported\n");
		return -ENOTSUPP;
	}

	if (!emsync_fps_update && dfps_caps.type == DSI_DFPS_IMMEDIATE_CLK) {
				    ...
				    u32 flags)
{
	...
	if (mode->dsi_mode_flags &
			(DSI_MODE_FLAG_DFPS |
			 DSI_MODE_FLAG_VRR |
			 DSI_MODE_FLAG_EMSYNC_FPS_SWITCH)) {
		display_for_each_ctrl(i, display) {
			ctrl = &display->ctrl[i];
...
#ifdef CONFIG_VIS_DISPLAY
	if (is_mi_dev_support_nova()) {
		vis_display_ops_send_mipi_cmd_notify(mi_dsi_display_cmd_write);
	}
#endif
...
#ifdef MI_DISPLAY_MODIFY
	SDE_ATRACE_BEGIN("dsi_display_get_info");
#endif
	mutex_lock(&display->display_lock);
...
error:
	mutex_unlock(&display->display_lock);
#ifdef MI_DISPLAY_MODIFY
	SDE_ATRACE_END("dsi_display_get_info");
#endif
	return rc;
}
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (is_skip_op_required(display)) {
		...
		if (display->panel->mi_cfg.feature_val[DISP_FEATURE_DBI] &&
			...
			if ((mi_get_panel_id_by_dsi_panel(display->panel) == P11U_PANEL_PA)) {
	...
	if ((mode->priv_info->dsc_enabled ||
		...
		(mi_get_panel_id_by_dsi_panel(panel) == P2_PANEL_SA || mi_get_panel_id_by_dsi_panel(panel) == Q200_PANEL_SA)) {
