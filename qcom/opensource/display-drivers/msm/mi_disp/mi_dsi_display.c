			...
			const char *type, u32 value)
{
	...
	if (dsi_ctrl->host_config.panel_mode == DSI_OP_CMD_MODE) {
		...
		dsi_ctrl->hw.ops.set_video_timing[dsi_ctrl->disp_op](&dsi_ctrl->hw, host_config);
	}
			...
			const char *type)
{
    ...
    host_config = &dsi_ctrl->host_config;
	if (dsi_ctrl->host_config.panel_mode != DSI_OP_CMD_MODE) {
		ret = dsi_ctrl->hw.ops.get_video_timing[dsi_ctrl->disp_op](&dsi_ctrl->hw, type);
	} else {
		...
		u32 *phy_timming, char *buf, size_t size, u8 phy_len)
{
	...
	if (phy && phy->hw.ops.get_phy_timing[MSM_DISP_OP_HWIO]) {
		phy->hw.ops.get_phy_timing[MSM_DISP_OP_HWIO](&phy->hw,
				phy_timming, phy_len);
	}
			...
			char *buf, size_t size)
{
	...
	pname = mi_get_panel_info(dsi_display->panel);
	if (pname) {
...
static int mi_dsi_display_re_read_wp_info(struct dsi_display *display)
{
	...
	if (mi_get_panel_id_by_dsi_panel(display->panel) == P2_PANEL_SA ) {
		for (i = 0; i < config->wp_cmds_rlen; i++) {
			rc = mi_dsi_display_cmd_read_panel_locked(display, cmd, &config->return_buf[i], 1);
			if (rc <= 0)
				DISP_ERROR("[DSI] Display command receive failed, rc=%d\n", rc);
			(*buf)++;
		}
		...
		char *buf, size_t size)
{
	...
	memset(config->return_buf, 0x0, sizeof(*config->return_buf));
	if (mi_get_panel_id_by_dsi_panel(display->panel) == P2_PANEL_SA ) {
		for (i = 0; i < config->cell_id_cmds_rlen; i++) {
			rc = mi_dsi_display_cmd_read_panel_locked(display, cmd, &config->return_buf[i], 1);
			if (rc <= 0) {
				DISP_ERROR("[DSI] Display command receive failed, rc=%d\n", rc);
				dsi_panel_release_panel_lock(display->panel);
...
{
	int policy_data = 0;
	if (kstrtoint(display_debug_policy, 16, &policy_data)) {
		DISP_ERROR("display_debug_policy kstrtoint error\n");
		return false;
	}
	DISP_INFO("display_debug_policy is 0x%02x\n", policy_data);
	/* when debug policy is 0x20, full dump not supported */
	if (policy_data != 0x20)
		return true;
	return false;
}
...
static void mi_display_pm_suspend_delayed_work_handler(struct kthread_work *work)
{
	...
	if (dsi_panel->panel_initialized && pmic_pwrkey_status == PMIC_PWRKEY_BARK_TRIGGER) {
		...
		if (mi_get_panel_id_by_dsi_panel(dsi_panel) == P2_PANEL_PB) {
