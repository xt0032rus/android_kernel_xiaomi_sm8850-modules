...
def gen_rmnet_headers(verbose, gen_dir, headers_install, unifdef, rmnet_include_uapi):
    error_count = 0
    for h in rmnet_include_uapi:
        rmnet_uapi_include_prefix = os.path.join(h.split('/include/uapi/')[0],
                                                 'include',
                                                 'uapi') + os.sep

        if not run_headers_install(
                verbose, gen_dir, headers_install, unifdef,
                rmnet_uapi_include_prefix, h): error_count += 1
    return error_count
...
def main():
    ...
    parser.add_argument(
            '--rmnet_include_uapi', required=True, nargs='*',
    ...
    if args.verbose:
        ...
        print('rmnet_include_uapi [%s]' % args.rmnet_include_uapi)
        print('headers_install [%s]' % args.headers_install)
        print('unifdef [%s]' % args.unifdef)

    return gen_rmnet_headers(args.verbose, args.gen_dir,
            args.headers_install, args.unifdef, args.rmnet_include_uapi)
